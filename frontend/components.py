...
{posts_html}
    ...
    with tab4:
        ...
        # ---------------------------------------------------------
      # ---------------------------------------------------------
        # THẾ GIỚI 1: DANH MỤC CHIẾN LƯỢC DÀI HẠN (ĐÃ LẮP RAM CHỐNG LAG TÚT TÙN TỤT)
        # ---------------------------------------------------------
        with sub_tab2:
            ...
            if 'port_cached_df' not in st.session_state or time.time() - st.session_state.get('port_cache_time', 0) > 900:
                with st.spinner("Đang đồng bộ dữ liệu Danh mục Dài hạn... (Chỉ mất vài giây cho lần đầu)"):
                    ...
                    if not portfolio_data:
                        ...
                        # Chạy vòng lặp lấy giá 1 lần duy nhất cho tất cả mã
            ...
            # ==========================================
            # BƯỚC 2: VẼ GIAO DIỆN LẤY TỪ RAM SIÊU TỐC
            # ==========================================
            ...
            if cached_port.empty:
                ...
                with col_s3:
                    st.markdown(f"<div style='padding: 16px; border-radius: 8px; border: 1px solid #EAECEF; text-align: center;'><div style='color: #707A8A; font-size: 11px; font-weight: 700; text-transform: uppercase;'>Lãi/Lỗ T.Tế</div><div style='font-size: 20px; font-weight: 800; color: {color_act};'>{avg_act_str}</div></div>", unsafe_allow_html=True)
                with col_s4:
