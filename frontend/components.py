               ...
               # ==========================================
                ...
                if 'port_cached_df' not in st.session_state or time.time() - st.session_state.get('port_cache_time', 0) > 900:
                    with st.spinner("Đang đồng bộ dữ liệu Danh mục Dài hạn (Bản gốc ổn định)..."):
                        ...
                        # --- 1. KÉO DỮ LIỆU TỪ SHEET THỦ CÔNG (BẬT RADAR TÌM LỖI) ---
                        manual_dict = {}
                        try:
                            manual_data = fetch_manual_price_db()
                            # Dòng này sẽ in thẳng dữ liệu nó kéo được ra màn hình cho Sếp thấy!
                            st.warning(f"🔍 Dữ liệu thô từ Sheet kéo về được là: {manual_data}") 
                            if manual_data:
                                for item in manual_data:
                                    row_vals = list(item.values())
                                    if len(row_vals) >= 2:
                                        tk = str(row_vals[0]).strip().upper()
                                        if tk:
                                            pr_str = str(row_vals[1]).replace(',', '').replace('.', '').replace(' ', '').strip()
                                            try:
                                                manual_dict[tk] = float(pr_str)
                                            except: pass
                        except Exception as e:
                            # Nếu có lỗi, nó sẽ la làng lên bằng màu đỏ chót!
                            st.error(f"🚨 TÌM THẤY THỦ PHẠM RỒI SẾP ƠI: {e}")
                        # -------------------------------------------
