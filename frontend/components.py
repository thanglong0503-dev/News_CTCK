...
{posts_html}
    ...
    with tab4:
        ...
        # ---------------------------------------------------------
      # ---------------------------------------------------------
        # THẾ GIỚI 1: DANH MỤC CHIẾN LƯỢC DÀI HẠN (CÔNG NGHỆ CHỐNG LAG @st.fragment)
        # ---------------------------------------------------------
        with sub_tab2:
            ...
            def render_long_term_portfolio():
                ...
                # ==========================================
                # BƯỚC 1: CACHE DATA VÀO RAM (15 PHÚT TẢI 1 LẦN)
                # ==========================================
                if 'port_cached_df' not in st.session_state or time.time() - st.session_state.get('port_cache_time', 0) > 900:
                    with st.spinner("Đang đồng bộ dữ liệu Danh mục Dài hạn... (Chỉ mất vài giây cho lần đầu)"):
                            ...
                            for _, row in df_port.iterrows():
                                ...
                                try:
                                    ...
                                    if not hist.empty:
                                        ...
                                        highest_price, lowest_price = hist['High'].max(), hist['Low'].min()
                                        
                                        if cp < 1000 and cp > 0: 
                                            cp *= 1000; highest_price *= 1000; lowest_price *= 1000
                                except: pass
                                ...
                                elif highest_price >= tgt_p and tgt_p > 0: statuses.append("✔️ Đã Đạt Target")
                                elif rec_p > 0 and lowest_price > 0 and lowest_price <= rec_p * 0.93: statuses.append("❌ Đã Chạm Cắt Lỗ")
                ...
                # ==========================================
                # BƯỚC 2: VẼ GIAO DIỆN TỪ RAM SIÊU TỐC
                # ==========================================
                ...
                # THÊM KEY CHO BỘ LỌC ĐỂ NÓ HOẠT ĐỘNG MƯỢT MÀ TRONG FRAGMENT
                ...
                with col_s1: st.markdown(f"<div style='padding: 16px; border-radius: 8px; border: 1px solid #EAECEF; text-align: center;'><div style='color: #707A8A; font-size: 11px; font-weight: 700; text-transform: uppercase;'>Số lượng mã</div><div style='font-size: 20px; font-weight: 800; color: #1E2329;'>{num_stocks}</div></div>", unsafe_allow_html=True)
                with col_s2: st.markdown(f"<div style='padding: 16px; border-radius: 8px; border: 1px solid #EAECEF; text-align: center;'><div style='color: #707A8A; font-size: 11px; font-weight: 700; text-transform: uppercase;'>Kỳ vọng TB</div><div style='font-size: 20px; font-weight: 800; color: #1E2329;'>{avg_exp_str}</div></div>", unsafe_allow_html=True)
                with col_s3: st.markdown(f"<div style='padding: 16px; border-radius: 8px; border: 1px solid #EAECEF; text-align: center;'><div style='color: #707A8A; font-size: 11px; font-weight: 700; text-transform: uppercase;'>Lãi/Lỗ T.Tế</div><div style='font-size: 20px; font-weight: 800; color: {color_act};'>{avg_act_str}</div></div>", unsafe_allow_html=True)
                with col_s4:
                    dat_target_count = filtered_port['Auto_Status'].tolist().count('✔️ Đã Đạt Target')
                    st.markdown(f"<div style='padding: 16px; border-radius: 8px; border: 1px solid #EAECEF; text-align: center;'><div style='color: #707A8A; font-size: 11px; font-weight: 700; text-transform: uppercase;'>Đã chạm Target</div><div style='font-size: 20px; font-weight: 800; color: #0ECB81;'>{dat_target_count}/{num_stocks}</div></div>", unsafe_allow_html=True)
        ...
        # ---------------------------------------------------------
      # ---------------------------------------------------------
        # THẾ GIỚI 2: DÒNG THỜI GIAN KHUYẾN NGHỊ (ĐỘNG CƠ BATCHING - CÂN 1000 BÁO CÁO)
        # ---------------------------------------------------------
        with sub_tab1:
            st.markdown("<br>", unsafe_allow_html=True)
                ...
                if 'rep_cached_df' not in st.session_state or time.time() - st.session_state.get('rep_cache_time', 0) > 900:
                    with st.spinner("Đang tải HÀNG TRĂM báo cáo và đồng bộ giá sỉ (Batching)..."):
                        ...
                        if not reports_data:
                            ...
                            # ==================================================
                            # ĐỘNG CƠ LẤY SỈ: GOM TẤT CẢ MÃ ĐỘC NHẤT LẠI ĐỂ TẢI 1 LẦN
                            # ==================================================
                            ...
                            if yf_tickers:
                                try:
                                    # Lệnh thần thánh: Tải 1 lúc toàn bộ lịch sử 1 năm của TẤT CẢ các mã (threads=True để tải song song)
                                    batch_data = yf.download(yf_tickers, period="1y", group_by='ticker', threads=True, progress=False)
                                except Exception as e:
                                    pass
                            ...
                            # ==================================================
                            # XỬ LÝ LẮP RÁP DỮ LIỆU SIÊU TỐC
                            # ==================================================
                            for _, r in df_temp.iterrows():
                                ...
                                # Rút trích giá từ "Kho sỉ" (batch_data)
                                if not batch_data.empty and yf_tickers:
                                    try:
                                        if len(yf_tickers) == 1:
                                            ticker_df = batch_data 
                                        elif isinstance(batch_data.columns, pd.MultiIndex) and yf_t in batch_data.columns.levels[0]:
                                            ticker_df = batch_data[yf_t]
                                        else:
                                            ticker_df = pd.DataFrame()
                                        
                                        if not ticker_df.empty:
                                            sliced_df = ticker_df.copy()
                                            # Khử múi giờ để dễ so sánh ngày
                                            if sliced_df.index.tz is not None:
                                                sliced_df.index = sliced_df.index.tz_localize(None)
                                                
                                            try:
                                                start_ts = pd.to_datetime(rec_date_str, format="%d/%m/%Y")
                                                sliced_df = sliced_df[sliced_df.index >= start_ts]
                                            except:
                                                pass # Nếu ngày sai thì lấy nguyên 1 năm
                                ...
                                if 'ĐẠT' in manual_status or 'TARGET' in manual_status: auto_statuses.append("✔️ ĐẠT TARGET")
                                    ...
                                    elif highest_price >= tgt_p and tgt_p > 0: auto_statuses.append("✔️ ĐẠT TARGET")
                                    elif rec_p > 0 and lowest_price <= rec_p * 0.93: auto_statuses.append("❌ CẮT LỖ")
