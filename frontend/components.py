...
# =========================================================
      ...
      # ---------------------------------------------------------
        ...
        # ==========================================
        # 2. VẼ GIAO DIỆN (KÉO TRỰC TIẾP TỪ GOOGLE SHEETS)
        # ==========================================
        import pandas as pd
        import math
        from datetime import datetime
        ...
        # -----------------------------------------------------------
        # VŨ KHÍ TỐI THƯỢNG: ĐÂM THẲNG VÀO GOOGLE SHEETS
        # Bùa @st.cache_data(ttl=300) giúp lưu tạm data 5 phút, Sếp chuyển Tab qua lại sẽ không bị lag load lại Google Sheet liên tục.
        # -----------------------------------------------------------
        @st.cache_data(ttl=300) 
        def pull_direct_from_gsheets():
            # 🛑 SẾP DÁN CODE KÉO DATA TỪ GOOGLE SHEETS VÀO ĐÂY 🛑
            # Ví dụ cách 1: Nếu Sếp dùng link CSV
            # url = "https://docs.google.com/spreadsheets/d/..../export?format=csv&gid=..."
            # return pd.read_csv(url)
            
            # Ví dụ cách 2: Nếu Sếp có hàm sẵn như `load_reports_db()`
            # return load_reports_db()
            
            # --- Tạm thời Emo gắp từ session_state (nếu có) để code không bị sập ---
            if 'rep_cached_df' in st.session_state and not st.session_state.rep_cached_df.empty:
                return st.session_state.rep_cached_df.copy()
            return pd.DataFrame() 

        # 1. GỌI DATA TRỰC TIẾP!
        df_rep = pull_direct_from_gsheets()
        
        # 2. Backup lại vào kho cho các Tab khác xài ké
        st.session_state['rep_cached_df'] = df_rep

        if df_rep.empty:
            st.error("🚨 Code giao diện đã sẵn sàng! Sếp nhớ nhét cái hàm kéo Google Sheets thật vào chỗ `pull_direct_from_gsheets` là data sẽ ùa về nhé!")
            return
