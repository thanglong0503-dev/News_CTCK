...
{posts_html}
    ...
    with tab4:
        ...
        # ---------------------------------------------------------
       # ---------------------------------------------------------
        # THẾ GIỚI 1: DANH MỤC CHIẾN LƯỢC DÀI HẠN (CÓ TRÍ NHỚ LỊCH SỬ)
        # ---------------------------------------------------------
        with sub_tab2:
            st.markdown("<br><div style='font-weight: 700; font-size: 16px; margin-bottom: 16px; color: #1E2329;'>Quản trị & Đánh giá Danh mục Tự động (Real-time)</div>", unsafe_allow_html=True)
            with st.spinner("Đang lục tìm lịch sử giá trên sàn chứng khoán để đánh giá độ chuẩn xác..."):
                ...
                if not portfolio_data:
                    ...
                    actual_returns = []
                    statuses = []
                    highest_prices = [] # Thêm bộ nhớ lưu giá cao nhất
                    ...
                    for _, row in filtered_port.iterrows():
                        ...
                        try:
                            ...
                            # Cố gắng dịch ngày khuyến nghị để lấy lịch sử từ ngày đó
                            try:
                                start_date = pd.to_datetime(rec_date_str, format="%d/%m/%Y").strftime('%Y-%m-%d')
                                hist = yf.Ticker(yf_ticker).history(start=start_date)
                            except:
                                # Nếu không có ngày hoặc nhập sai form, lấy lùi lại 6 tháng làm mặc định
                                hist = yf.Ticker(yf_ticker).history(period="6mo")
                                
                            if not hist.empty:
                                cp = hist['Close'].iloc[-1]           # Giá chốt phiên hôm nay
                                highest_price = hist['High'].max()    # Đỉnh cao nhất lịch sử từ lúc khuyến nghị
                                lowest_price = hist['Low'].min()      # Đáy thấp nhất lịch sử từ lúc khuyến nghị
                                
                                # Xử lý chênh lệch tỷ lệ giá của Yahoo Finance
                                if cp < 1000 and cp > 0: 
                                    cp = cp * 1000 
                        ...
                        if rec_p > 0 and cp > 0:
                            ret_percent = ((cp - rec_p) / rec_p) * 100
                            actual_returns.append(ret_percent)
                        else:
                            actual_returns.append(0)
                            
                        # THUẬT TOÁN ĐÁNH GIÁ THÔNG MINH (DÙNG ĐỈNH/ĐÁY LỊCH SỬ)
                        if highest_price >= tgt_p and tgt_p > 0:
                            statuses.append("✔️ Đã Đạt Target") # Từng chạm Target trong quá khứ là tính Thắng
                        elif rec_p > 0 and lowest_price <= rec_p * 0.93: 
                            statuses.append("❌ Đã Chạm Cắt Lỗ") # Từng rớt quá 7% là tính Thua
                        else:
                            statuses.append("⏳ Đang bám sát")

                    filtered_port['Current_Price'] = current_prices
                    filtered_port['Actual_Return'] = actual_returns
                    filtered_port['Auto_Status'] = statuses
                    filtered_port['Highest_Reached'] = highest_prices
                    ...
                    avg_actual = filtered_port['Actual_Return'].mean()
                    color_act = "#0ECB81" if avg_actual >= 0 else "#F6465D"
                    sign_act = "+" if avg_actual > 0 else ""
                    
                    col_s1, col_s2, col_s3, col_s4 = st.columns(4)
                    with col_s1:
                        st.markdown(f"<div style='padding: 16px; border-radius: 8px; border: 1px solid #EAECEF; text-align: center;'><div style='color: #707A8A; font-size: 11px; font-weight: 700; text-transform: uppercase;'>Số lượng mã</div><div style='font-size: 20px; font-weight: 800; color: #1E2329;'>{num_stocks}</div></div>", unsafe_allow_html=True)
                    with col_s2:
                        st.markdown(f"<div style='padding: 16px; border-radius: 8px; border: 1px solid #EAECEF; text-align: center;'><div style='color: #707A8A; font-size: 11px; font-weight: 700; text-transform: uppercase;'>Kỳ vọng TB</div><div style='font-size: 20px; font-weight: 800; color: #1E2329;'>{avg_exp_str}</div></div>", unsafe_allow_html=True)
                    with col_s3:
                        st.markdown(f"<div style='padding: 16px; border-radius: 8px; border: 1px solid #EAECEF; text-align: center;'><div style='color: #707A8A; font-size: 11px; font-weight: 700; text-transform: uppercase;'>Lãi/Lỗ T.Tế (Hiện tại)</div><div style='font-size: 20px; font-weight: 800; color: {color_act};'>{sign_act}{avg_actual:.1f}%</div></div>", unsafe_allow_html=True)
                    with col_s4:
                        st.markdown(f"<div style='padding: 16px; border-radius: 8px; border: 1px solid #EAECEF; text-align: center;'><div style='color: #707A8A; font-size: 11px; font-weight: 700; text-transform: uppercase;'>Đã chạm Target</div><div style='font-size: 20px; font-weight: 800; color: #0ECB81;'>{statuses.count('✔️ Đã Đạt Target')}/{num_stocks}</div></div>", unsafe_allow_html=True)
                    ...
                    st.dataframe(
                        ...
                        column_config={
                            ...
                            "Actual_Return": st.column_config.NumberColumn("Lãi/Lỗ T.Tế", format="%.1f %%"),
                ...
                if not reports_data:
                    ...
                    with col_list:
                        ...
                        for _, r in filtered_rep.iterrows():
                            ...
                            # CỰC KỲ QUAN TRỌNG: Lấy trạng thái thủ công từ Google Sheets
                            manual_status = str(r.get('Status', '')).strip().upper() 
                            ...
                            # --- CƠ CHẾ ĐÁNH GIÁ (ƯU TIÊN LỆNH CHỈNH TAY) ---
                            if 'ĐẠT' in manual_status or 'TARGET' in manual_status:
                                auto_statuses.append("✔️ ĐẠT TARGET") # Sếp bảo Đạt là Đạt
                            elif 'CẮT' in manual_status or 'LỖ' in manual_status:
                                auto_statuses.append("❌ CẮT LỖ")      # Sếp bảo Cắt là Cắt
                            else:
                                # Sếp chưa phán quyết -> Cho AI tự tính
                                if cp == 0 or lowest_price == 0:
                                    # Lỗi API hoặc không tải được data -> Đẩy về Theo dõi chờ check tay
                                    auto_statuses.append("⏳ ĐANG THEO DÕI")
                                elif highest_price >= tgt_p and tgt_p > 0:
                                    # Trong quá khứ từng chạm Target -> Ghi nhận Đạt
                                    auto_statuses.append("✔️ ĐẠT TARGET")
                                elif rec_p > 0 and lowest_price <= rec_p * 0.93: 
                                    # Từng rớt 7% từ lúc khuyến nghị -> Ghi nhận Cắt lỗ
                        ...
                        if paged_rep.empty:
                            ...
                            for _, r in paged_rep.iterrows():
                                ...
                                # Mở rộng từ khóa để hiểu nhiều loại Action hơn
                    ...
                    with col_leaderboard:
                        ...
                        # --- 1. TÍNH TOÁN WIN RATE TỪ TRẠNG THÁI ---
