                    ...
                    with st.spinner("Đang đồng bộ dữ liệu Danh mục Dài hạn... (Chỉ mất vài giây cho lần đầu)"):
                        ...
                        if not portfolio_data:
                            ...
                            for _, row in df_port.iterrows():
                                ...
                                # THUẬT TOÁN ĐÁNH GIÁ CỦA DANH MỤC DÀI HẠN
                            if cp == 0 or lowest_price == 0: 
                                statuses.append("⏳ Đang bám sát") 
                            elif highest_price >= tgt_p and tgt_p > 0: 
                                statuses.append("✔️ Đã Đạt Target")
                            elif rec_p > 0 and lowest_price > 0 and lowest_price <= rec_p * 0.88: # Nới 12%
                                if cp >= rec_p * 0.98: # Ân xá nếu giá đã hồi phục
                                    statuses.append("⏳ Đang bám sát")
                                else:
                ...
                if 'rep_cached_df' not in st.session_state or time.time() - st.session_state.get('rep_cache_time', 0) > 900:
                    with st.spinner("Đang tải HÀNG TRĂM báo cáo và đồng bộ giá sỉ (Batching)..."):
                        ...
                        if not reports_data:
                            ...
                            if yf_tickers:
                                try:
                                    batch_data = yf.download(yf_tickers, period="6y", group_by='ticker', threads=False, progress=False)
                            ...
                            for _, r in df_temp.iterrows():
                                ...
                                # CƠ CHẾ ĐÁNH GIÁ THÔNG MINH (CÓ LUẬT ÂN XÁ & NỚI BIÊN ĐỘ)
                                if 'ĐẠT' in manual_status or 'TARGET' in manual_status: auto_statuses.append("✔️ ĐẠT TARGET")
                                    ...
                                    elif rec_p > 0 and lowest_price <= rec_p * 0.88: # Nới cắt lỗ lên 12% (0.88)
                                        # LUẬT ÂN XÁ: Nếu từng rớt 12% nhưng HIỆN TẠI đã kéo lại sát giá mua (phục hồi trên 98% giá gốc)
                                        if cp >= rec_p * 0.98: 
                                            auto_statuses.append("⏳ ĐANG THEO DÕI") 
                                        else:
