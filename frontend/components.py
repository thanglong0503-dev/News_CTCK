...
# --- TAB 5: SO SÁNH DỊCH VỤ ---
    with tab5:
        ...
        with st.spinner("Đang trích xuất dữ liệu so sánh..."):
            ...
            if not broker_data:
                st.info("Chưa có dữ liệu so sánh từ LINANCE_DB")
            else:
                ...
                for b in broker_data:
                    ...
                    # Dịch toàn bộ HTML ra sát lề trái để Streamlit không tưởng nhầm là Code Block
                    cards_html += f"""<div class="broker-card">
<div class="broker-name">{b.get('Broker_Name', 'N/A')} <span class="broker-badge">ƯU ĐÃI</span></div>
<div class="stat-line"><span class="stat-lbl">Phí giao dịch</span><span class="stat-val" style="color: #FF6B00;">{b.get('Trading_Fee', 'N/A')}</span></div>
<div class="stat-line"><span class="stat-lbl">Lãi suất Margin</span><span class="stat-val">{b.get('Margin_Rate', 'N/A')}</span></div>
<div class="stat-line"><span class="stat-lbl">Nguồn Margin</span><span class="stat-val" style="color: {pool_color};">{b.get('Margin_Pool', 'N/A')}</span></div>
<div class="pros-box">🎯 {b.get('Pros', 'Liên hệ để biết thêm chi tiết')}</div>
