...
{posts_html}
    ...
    with tab4:
        ...
        # ---------------------------------------------------------
       # ---------------------------------------------------------
        # THẾ GIỚI 1: DANH MỤC CHIẾN LƯỢC DÀI HẠN (ĐÃ FIX LỖI GIÁ = 0)
        # ---------------------------------------------------------
        with sub_tab2:
            ...
            with st.spinner("Đang lục tìm lịch sử giá trên sàn chứng khoán để đánh giá độ chuẩn xác..."):
                ...
                if not portfolio_data:
                    ...
                    for _, row in filtered_port.iterrows():
                        ...
                        # FIX LỖI 0Đ: Nếu bằng 0 thì chuyển thành None để bảng không hiện số ảo
                        ...
                        if rec_p > 0 and cp > 0:
                            ...
                            actual_returns.append(None) # FIX: Chuyển thành None để không bị kéo tụt trung bình
                            
                        # THUẬT TOÁN ĐÁNH GIÁ (FIX LỖI ÉP CẮT LỖ)
                        if cp == 0 or lowest_price == 0:
                            statuses.append("⏳ Đang bám sát") # Lỗi API hoặc không tải được -> Giữ nguyên theo dõi
                        elif highest_price >= tgt_p and tgt_p > 0:
                            statuses.append("✔️ Đã Đạt Target")
                        elif rec_p > 0 and lowest_price > 0 and lowest_price <= rec_p * 0.93: 
                    ...
                    # TÍNH LẠI TRUNG BÌNH LÃI LỖ (BỎ QUA CÁC MÃ BỊ LỖI API)
                    valid_returns = [r for r in actual_returns if r is not None]
                    if valid_returns:
                    ...
                    with col_s3:
                        st.markdown(f"<div style='padding: 16px; border-radius: 8px; border: 1px solid #EAECEF; text-align: center;'><div style='color: #707A8A; font-size: 11px; font-weight: 700; text-transform: uppercase;'>Lãi/Lỗ T.Tế (Hiện tại)</div><div style='font-size: 20px; font-weight: 800; color: {color_act};'>{avg_act_str}</div></div>", unsafe_allow_html=True)
                    with col_s4:
