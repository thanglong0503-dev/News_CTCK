...
# --- TAB 5: SO SÁNH DỊCH VỤ ---
    with tab5:
        ...
        st.markdown("<div style='color: #474D57; font-size: 14px; margin-bottom: 24px;'>Dữ liệu được cập nhật trực tiếp từ hệ thống quản trị LINANCE.</div>", unsafe_allow_html=True)
        ...
        with st.spinner("Đang trích xuất dữ liệu so sánh..."):
            ...
            if not broker_data:
                ...
                # CSS dùng Flexbox và khóa cứng chiều rộng 320px để tạo thẻ dọc nhỏ gọn
                css_broker = """
                <style>
                ...
                .b-name { font-size: 18px; font-weight: 800; color: #1E2329; margin-bottom: 16px; display: flex; align-items: center; justify-content: space-between; }
                .b-badge { background: #FFF2E5; color: #FF6B00; font-size: 10px; padding: 4px 8px; border-radius: 4px; font-weight: 700; }
                ...
                for b in broker_data:
                    ...
                    # SIÊU QUAN TRỌNG: Viết toàn bộ trên 1 dòng duy nhất để Streamlit không làm lỗi HTML
                    cards_html += f"""<div class="b-card"><div class="b-name"><span>{b.get('Broker_Name', 'N/A')}</span><span class="b-badge">ƯU ĐÃI</span></div><div class="b-stat"><span class="b-lbl">Phí giao dịch</span><span class="b-val" style="color: #FF6B00;">{b.get('Trading_Fee', 'N/A')}</span></div><div class="b-stat"><span class="b-lbl">Lãi suất Margin</span><span class="b-val">{b.get('Margin_Rate', 'N/A')}</span></div><div class="b-stat"><span class="b-lbl">Nguồn Margin</span><span class="b-val" style="color: {pool_color};">{b.get('Margin_Pool', 'N/A')}</span></div><div class="b-pros">🎯 {b.get('Pros', 'Liên hệ chi tiết')}</div><div class="b-upd">Cập nhật: {b.get('Last_Updated', 'N/A')}</div></div>"""
                
                # Nối thẳng HTML một cách mượt mà
