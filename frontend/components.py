               ...
               # ==========================================
                ...
                if 'port_cached_df' not in st.session_state or time.time() - st.session_state.get('port_cache_time', 0) > 900:
                    with st.spinner("Đang đồng bộ dữ liệu Danh mục Dài hạn (Bản gốc ổn định)..."):
                        ...
                        # --- ĐỘNG CƠ XÉT NGHIỆM TÌM LỖI (CHỈ ĐỂ TEST) ---
                        manual_dict = {}
                        try:
                            # Tạm thời vô hiệu hóa bộ nhớ đệm (Cache) của hệ thống bằng lệnh này
                            if 'rep_cache_time' in st.session_state: del st.session_state['rep_cache_time']
                            
                            manual_data = fetch_manual_price_db()
                            st.info(f"1️⃣ Dữ liệu Backend trả về: {manual_data}")
                            ...
                            if manual_data and len(manual_data) > 1:
                                for row in manual_data[1:]:
                                    if len(row) >= 2:
                                        ...
                                        st.warning(f"2️⃣ Đang xử lý mã: [{tk}] - Giá chữ: [{pr_str}]")
                                        try:
                                            manual_dict[tk] = float(pr_str)
                                        except Exception as e: 
                                            st.error(f"❌ Lỗi biến thành số ở mã {tk}: {e}")
                            
                            st.success(f"3️⃣ Từ điển giá cuối cùng: {manual_dict}")
                        except Exception as e:
                            st.error(f"❌ Lỗi sập nguồn toàn tập: {e}")
                        # ------------------------------------------------
                        # -------------------------------------------
