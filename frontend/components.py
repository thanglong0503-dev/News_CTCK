...
{posts_html}
    ...
    with tab4:
      ...
      # ---------------------------------------------------------
        ...
        # ---------------------------------------------------------
       # ---------------------------------------------------------
        # THẾ GIỚI 2: DÒNG THỜI GIAN KHUYẾN NGHỊ (CÔNG NGHỆ CHỐNG LAG @st.fragment)
        # ---------------------------------------------------------
        with sub_tab1:
            st.markdown("<br>", unsafe_allow_html=True)
            
            # Khởi tạo biến trang ở ngoài để giữ trí nhớ
            if 'report_page' not in st.session_state: st.session_state.report_page = 1

            # BÙA CHÚ BỌC GIAO DIỆN LẠI THÀNH 1 MẢNH ĐỘC LẬP
            @st.fragment
                    ...
                    if total_pages > 1:
                        ...
                        with pag_cols[1]:
                            if st.button("◀ Trước", disabled=(st.session_state.report_page <= 1), use_container_width=True, key="rep_prev"):
                                ...
                                # KHÔNG DÙNG st.rerun() NỮA! FRAGMENT SẼ TỰ CHẠY LẠI
                        with pag_cols[2]: 
            ...
            # KÍCH HOẠT TIỂU VŨ TRỤ CHỐNG LAG LÊN MÀN HÌNH
