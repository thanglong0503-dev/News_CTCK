...
{posts_html}
    ...
    with tab4:
        ...
        # ---------------------------------------------------------
      # ---------------------------------------------------------
        # THẾ GIỚI 1: DANH MỤC CHIẾN LƯỢC DÀI HẠN (UI TONE CAM CAO CẤP)
        # ---------------------------------------------------------
        with sub_tab2:
            ...
            def render_long_term_portfolio():
                ...
                # ==========================================
                # 1. CACHE DATA VÀO RAM (15 PHÚT TẢI 1 LẦN)
                # ==========================================
        ...
        # ---------------------------------------------------------
      # ---------------------------------------------------------
        # THẾ GIỚI 2: DÒNG THỜI GIAN KHUYẾN NGHỊ (ĐÃ ÉP PHẲNG HTML CHỐNG LỖI)
        # ---------------------------------------------------------
        with sub_tab1:
            st.markdown("<br>", unsafe_allow_html=True)
                ...
                with col_leaderboard:
                    ...
                    if not filtered_rep.empty:
                        ...
                        if not valid_upside.empty:
                            ...
                            top_upside_html = f"<div style='background: #3B424A; border-radius: 8px; padding: 12px; border-left: 3px solid #0ECB81;'><div style='display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;'><span style='font-size: 18px; font-weight: 800; color: #fff; font-family: \"SF Mono\", Consolas, monospace;'>{t_tkr}</span><span style='background: rgba(14, 203, 129, 0.2); color: #0ECB81; padding: 4px 8px; border-radius: 4px; font-size: 12px; font-weight: 800;'>+{t_up:.1f}% Upside</span></div><div style='display: flex; justify-content: space-between; font-size: 12px; color: #848E9C; margin-bottom: 4px;'><span>Giá HT: <b style='color: #fff;'>{t_cp}</b></span><span>Target: <b style='color: #FF6B00;'>{t_tp}</b></span></div><div style='font-size: 10px; color: #707A8A; text-align: right;'>Đề xuất bởi: {t_brk}</div></div>"

                        radar_html = f"<div style='margin-top: 16px; padding: 16px; background: linear-gradient(135deg, #1E2329, #14171A); border-radius: 12px; border: 1px solid #474D57; color: white; box-shadow: 0 8px 16px rgba(0,0,0,0.15);'><div style='font-size: 14px; font-weight: 800; color: #FF6B00; margin-bottom: 16px; display: flex; align-items: center; gap: 8px;'><span style='font-size: 18px;'>🔥</span> RADAR CƠ HỘI (AI SCAN)</div><div style='margin-bottom: 20px;'><div style='font-size: 11px; color: #848E9C; text-transform: uppercase; font-weight: 700; margin-bottom: 8px;'>🚀 Cổ phiếu có dư địa tăng cao nhất</div>{top_upside_html}</div><div><div style='font-size: 11px; color: #848E9C; text-transform: uppercase; font-weight: 700; margin-bottom: 8px;'>🌡️ Nhiệt kế Khuyến nghị</div><div style='display: flex; height: 6px; border-radius: 4px; overflow: hidden; margin-bottom: 6px; background: #2B323A;'><div style='width: {buy_pct}%; background: #0ECB81;'></div><div style='width: {hold_pct}%; background: #F39C12;'></div><div style='width: {sell_pct}%; background: #F6465D;'></div></div><div style='display: flex; justify-content: space-between; font-size: 10px; color: #848E9C; font-weight: 700;'><span style='color: #0ECB81;'>MUA {buy_pct:.0f}%</span><span style='color: #F39C12;'>GIỮ {hold_pct:.0f}%</span><span style='color: #F6465D;'>BÁN {sell_pct:.0f}%</span></div></div></div>"
