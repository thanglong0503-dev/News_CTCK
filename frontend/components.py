...
{posts_html}
    ...
    with tab4:
        ...
        # ---------------------------------------------------------
        # ---------------------------------------------------------
        # THẾ GIỚI 1: DANH MỤC CHIẾN LƯỢC DÀI HẠN (REAL-TIME AUTO)
        # ---------------------------------------------------------
        with sub_tab2:
            ...
            with st.spinner("Đang kết nối sàn chứng khoán để cập nhật giá Real-time..."):
                portfolio_data = fetch_portfolio_db()
                if not portfolio_data:
                    st.info("💡 Chưa có dữ liệu. Ngươi hãy tạo tab 'PORTFOLIO_DB' trên Sheets nhé!")
                    st.markdown("**Các cột cần có:** `Portfolio_Name` | `Sector` | `Ticker` | `Company` | `Rec_Price` | `Target_Price` | `Expected_Return` | `Link`")
                else:
                    import pandas as pd
                    import yfinance as yf
                    ...
                    # 2. LỌC DATA VÀ LẤY GIÁ THỊ TRƯỜNG HIỆN TẠI (YFINANCE)
                    ...
                    for _, row in filtered_port.iterrows():
                        ...
                        # Gọi API yfinance lấy giá chốt phiên gần nhất
                        cp = 0
                        try:
                            ...
                            if not hist.empty:
                                ...
                                # Xử lý chênh lệch tỷ lệ giá (Yahoo Finance hay chia 1000)
                                if cp < 1000 and cp > 0: cp = cp * 1000 
                        except:
                            cp = 0
                            
                        current_prices.append(cp)
                        
                        # Tính Lãi/Lỗ Thực tế (Actual Return)
                        if rec_p > 0 and cp > 0:
                        ...
                        # Tự động đánh giá trạng thái (Auto-Status)
                        if cp >= tgt_p and tgt_p > 0:
                            statuses.append("✔️ Đạt Target")
                        elif rec_p > 0 and cp <= rec_p * 0.93: # Lỗ trên 7% thì báo Cắt Lỗ
                            statuses.append("❌ Cắt Lỗ")
                        else:
                            statuses.append("⏳ Đang theo dõi")

                    # Gắn dữ liệu tính toán vào Bảng
                    ...
                    with col_s2:
                        st.markdown(f"<div style='padding: 16px; border-radius: 8px; border: 1px solid #EAECEF; text-align: center;'><div style='color: #707A8A; font-size: 11px; font-weight: 700; text-transform: uppercase;'>Kỳ vọng TB (Dự phóng)</div><div style='font-size: 20px; font-weight: 800; color: #1E2329;'>{avg_exp_str}</div></div>", unsafe_allow_html=True)
                    with col_s3:
                        st.markdown(f"<div style='padding: 16px; border-radius: 8px; border: 1px solid #EAECEF; text-align: center;'><div style='color: #707A8A; font-size: 11px; font-weight: 700; text-transform: uppercase;'>Lãi/Lỗ Thực tế (Hiện tại)</div><div style='font-size: 20px; font-weight: 800; color: {color_act};'>{sign_act}{avg_actual:.1f}%</div></div>", unsafe_allow_html=True)
                    with col_s4:
                        st.markdown(f"<div style='padding: 16px; border-radius: 8px; border: 1px solid #EAECEF; text-align: center;'><div style='color: #707A8A; font-size: 11px; font-weight: 700; text-transform: uppercase;'>Trạng thái Danh mục</div><div style='font-size: 20px; font-weight: 800; color: #FF6B00;'>Đang bám sát</div></div>", unsafe_allow_html=True)

                    st.markdown("<br>", unsafe_allow_html=True)

                    # 4. HIỂN THỊ BẢNG (ĐÃ TÍCH HỢP ĐỦ CỘT)
                    st.dataframe(
                        filtered_port,
                        column_config={
                            "Portfolio_Name": None, # Ẩn đi vì đã có trên tiêu đề
                            "Sector": None, # Ẩn cột ngành cho bảng đỡ chật
                            "Ticker": st.column_config.TextColumn("Mã CP", width="small"),
                            "Company": st.column_config.TextColumn("Doanh nghiệp", width="medium"),
                            "Rec_Price": st.column_config.NumberColumn("Giá Khuyến Nghị", format="%d ₫"),
                            ...
                            "Auto_Status": st.column_config.TextColumn("Trạng thái (AI)"),
                            "Link": st.column_config.LinkColumn("Báo cáo", display_text="Xem ↗")
                        },
                        hide_index=True,
                    ...
                    with col_leaderboard:
                        ...
                        if closed_df.empty:
                            ...
                            for idx, row in win_stats.iterrows():
                                ...
                                # Gắn HTML trên 1 dòng duy nhất để chống lỗi Markdown
                        ...
                        # Gắn HTML trên 1 dòng duy nhất để chống lỗi Markdown
                        final_html = f"""<div style='background: #FAFAFA; border: 1px solid #EAECEF; border-radius: 8px; padding: 20px; position: relative; margin-top: 10px;'><div style="font-size: 12px; color: #707A8A; margin-bottom: 20px; line-height: 1.5;">Tỷ lệ Win Rate được AI tự động tính toán dựa trên số báo cáo đã chạm Target hoặc Cắt lỗ.</div>{leaderboard_html}<div style="margin-top: 24px; padding: 12px; background: #E6FFF3; border-radius: 6px; border: 1px dashed #0ECB81;"><div style="font-size: 11px; color: #0ECB81; font-weight: 800; text-transform: uppercase; margin-bottom: 4px;">🤖 AI Consensus</div><div style="font-size: 13px; color: #1E2329; font-weight: 600;">{consensus_html}</div></div></div>"""
